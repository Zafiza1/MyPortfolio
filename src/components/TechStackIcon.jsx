...
const TechStackIcon = ({ TechStackIcon, Language }) => {
  // Icons that need brightness adjustment
  const darkIcons = ['typescript', 'php', 'postgresql', 'docker', 'github'];
  const isDarkIcon = darkIcons.some(icon => TechStackIcon.toLowerCase().includes(icon));
    ...
    <div className="group p-6 rounded-2xl bg-slate-800/50 hover:bg-slate-700/50 transition-all duration-300 ease-in-out flex flex-col items-center justify-center gap-3 hover:scale-105 cursor-pointer shadow-lg hover:shadow-xl">
      <div className="relative">
        ...
        <img 
          src={TechStackIcon} 
          alt={`${Language} icon`} 
          className={`relative h-16 w-16 md:h-20 md:w-20 transform transition-transform duration-300 ${isDarkIcon ? 'brightness-125' : ''}`}
        />
      </div>

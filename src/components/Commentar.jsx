...
import { getComments, createComment, uploadImage } from '../api';
...
const Komentar = () => {
    ...
    useEffect(() => {
        const fetchPinnedComment = async () => {
            try {
                ...
                const pinned = allComments.find(c => c.is_pinned === true);
                if (pinned) {
    ...
    useEffect(() => {
        const fetchComments = async () => {
            try {
                const allComments = await getComments();
                const regularComments = allComments.filter(c => c.is_pinned === false);
    ...
    const uploadCommentImage = useCallback(async (imageFile) => {
    ...
    const handleCommentSubmit = useCallback(async ({ newComment, userName, imageFile }) => {
        ...
        try {
            const profileImageUrl = await uploadCommentImage(imageFile);
